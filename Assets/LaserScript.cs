...
//	{
		...
		{
			BulletManager bulletManagerScript = (BulletManager)bulletManager.GetComponent (typeof(BulletManager));
			GameObject bullet = bulletManagerScript.getAvailableBullet();

			// another way to call(invoke) a function of the game object's script.
			if (bullet == null) {
				return; // no available bullet yet.
			}
...
//			sphere.transform.position = hitInfo.point;
			...
			bulletShooter.shoot (ray, gameObject);
			if (hit) 
			{
				Debug.Log ("LaserScript::Update(): hitInfo.transform.gameObject.name = " + hitInfo.transform.gameObject.name);
